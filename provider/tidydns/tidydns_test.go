...
func TestFindSuitableZones(t *testing.T) {
	z := findSuitableZones(zones, "test1.tidydns1.com")
	assert.Equal(t, "tidydns1.com", z.Name)

	z = findSuitableZones(zones, "test2.sub.tidydns2.com")
	assert.Equal(t, "sub.tidydns2.com", z.Name)

	z = findSuitableZones(zones, "test-sub.tidydns2.com")
	assert.Equal(t, "tidydns2.com", z.Name)

	z = findSuitableZones(zones, "extdns.test-sub.tidydns2.com")
	assert.Equal(t, "tidydns2.com", z.Name)

	z = findSuitableZones(zones, "tidydns2.com")
	assert.Equal(t, "tidydns2.com", z.Name)
}
...
func TestTidyDNSRecords(t *testing.T) {
	...
	client.On("ListRecords", mock.Anything, 1).Return(zone1, nil)
	client.On("ListRecords", mock.Anything, 2).Return(zone2, nil)
...
func TestTidyDNSApplyChanges(t *testing.T) {
	...
	client := &mockClient{}
	client.On("ListZones", mock.Anything).Return(zones, nil)
	client.On("FindRecord", mock.Anything, 1, "ttl", tidydns.RecordTypeA).Return(ttl, nil)
	client.On("FindRecord", mock.Anything, 1, "regular3", tidydns.RecordTypeA).Return(regular3, nil)
	client.On("FindRecord", mock.Anything, 1, "regular4", tidydns.RecordTypeA).Return(regular4, nil)
	client.On("FindRecord", mock.Anything, 1, "regular5", tidydns.RecordTypeA).Return(regular5, nil)
	client.On("CreateRecord", mock.Anything, 1, tidydns.RecordInfo{ID: 0, Type: 0, Name: "regular1", Description: "", Destination: "1.1.1.1", TTL: 0, Status: 0, Location: 0}).Return(1, nil)
	client.On("CreateRecord", mock.Anything, 1, tidydns.RecordInfo{ID: 0, Type: 0, Name: "ttl", Description: "", Destination: "1.1.1.1", TTL: 100, Status: 0, Location: 0}).Return(2, nil)
	client.On("CreateRecord", mock.Anything, 1, tidydns.RecordInfo{ID: 0, Type: 0, Name: "regular2", Description: "", Destination: "1.1.1.2", TTL: 0, Status: 0, Location: 0}).Return(3, nil)
	client.On("UpdateRecord", mock.Anything, 1, 3, tidydns.RecordInfo{ID: 0, Type: 0, Name: "regular3", Description: "", Destination: "1.1.2.2", TTL: 100, Status: 0, Location: 0}).Return(nil)
	client.On("UpdateRecord", mock.Anything, 1, 4, tidydns.RecordInfo{ID: 0, Type: 0, Name: "regular4", Description: "", Destination: "1.1.2.2", TTL: 100, Status: 0, Location: 0}).Return(nil)
	client.On("DeleteRecord", mock.Anything, 1, 5).Return(nil)
	client.On("DeleteRecord", mock.Anything, 1, 42).Return(nil)
