...
// it: converts to lower case, removes space, encode to ASCII using Punycode, ensures there is a trailing dot
func normalizeDNSName(dnsName string) string {
	s, err := idna.ToASCII(strings.TrimSpace(strings.ToLower(dnsName)))
	if err != nil {
		log.Warnf(`Got error while parsing %s: %v`, dnsName, err)
	}
